...
# JerryPipe, but with an added QC step that remove duplicates 
    # Designed to process DNA sequencing data
...
inputFolder = "/n/scratch2/jy250/dnaFastq/"
...
def convertToSAM(commands_list, refGenome, fastqFile, fastqPaired = ""):
    ...
    commands_list.append("bwa mem -M {refGen} {fastq1} {fastq2}> {sam}".format(refGen = refGenome, fastq1 = fastqFile, fastq2 = fastqPaired, sam = samfile))
    return samfile
...
def Launch_JerryPipe(fastqFile, pairedFile = None):
    '''
    ...
    samFile = convertToSAM(commands_list, refGenome, fastqFile, fastqPaired = pairedFile) # Adds commands to commandList to convert 
